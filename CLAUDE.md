...
| `PseudoWinter.html` | v1.14.3 |
| `PseudoChaser.html` | v1.10.4 |
| `PsychoWinter1.0.html` | v1.0 |
| `plugins/strategies/MultiIndicator-Winter.html` | v1.76.0 |
| `plugins/strategies/MultiIndicator-Chaser.html` | v1.76.0 |
| `plugins/analytics/Permafrost-Winter.html` | v1.73.0 |
| `plugins/analytics/Ashfall-Chaser.html` | v1.73.0 |
| `plugins/modes/EverWinter.html` | v1.0.4 |
| `plugins/modes/SunChaser.html` | v1.0.4 |
| `plugins/multiplex/Speeder-Chaser.html` | v1.5.1 |
| `plugins/multiplex/Speeder-Winter.html` | v1.5.1 |
...
- **Cut preamble; don't name-drop identifiers in prose.** Skip throat-clearing like "Before I go further — this doesn't have as clean an explanation... let me lay out what I found." Say the plain version instead: "I'm not sure what's causing that, but here's how the code works." When explaining code to the user in conversation, use descriptive nicknames instead of literal variable/function names ("the chart", "the skew value", "the dedicated function" — not `_pfOiSvg`/`_miwOiSkew`/the real identifier) so the user isn't parsing symbol names to follow the explanation. Reserve exact identifiers for places the user needs them to locate code themselves (file:line references, code review comments) — not for conversational explanations of how something works.
- **Always update README.md at the end of a significant change.** A significant change is one that adds/removes a config toggle, panel, or user-visible behavior — not a pure bug fix with no new surface. Do this proactively, without waiting to be asked, following the existing Documentation Writing Style rules for README.

...
| `ocx>N` / `ocx<N` | Average seconds between orders in the sampled batch, tiered at N seconds (1s step by default). `ocx<N` requires an average gap at or below N seconds (busy order flow); `ocx>N` requires at or above N seconds (quiet order flow). Requires Permafrost/Ashfall OC Surveillance. |

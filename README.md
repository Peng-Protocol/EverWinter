...
Permafrost/Ashfall also include a Cross-tab **Leader/Follower DDH Failover** toggle for Leader/Follower. When enabled and the active leader enters a drawdown halt, the plugin temporarily turns **Follow Partner Bot** on for that instance so it stops leading live-liq surveillance. If the current instance was following and the partner leader enters drawdown, the plugin temporarily turns **Follow Partner Bot** off so this instance becomes the live-liq leader. When neither side is in drawdown, the plugin restores the user's previous Follow Partner Bot setting.
...
| **PEC Depth Step** (`miwPecDepthStep`/`micPecDepthStep`) | Index-point size of one depth tier for the collapsed-rank scorecard score (default 5). Strongest signal for `pecob`/`pecos`/momentum; noisier for reversion/mountain/delta since those don't scale linearly with distance from a threshold. `pecirr`'s tier uses this same step, measured as distance past **Irregularity Threshold** rather than past PEC Threshold High/Low. |
...
| **VSG Depth Step** (`miwVsgDepthStep`/`micVsgDepthStep`) | Index-point size of one depth tier for the collapsed-rank scorecard score (default 5). Same scale and same per-criterion caveats as PEC Depth Step. `vsgirr`'s tier uses this same step, measured as distance past **Irregularity Threshold** rather than past VSG Threshold High/Low. |
...
| **Regime Based Scoring** (`pfRegimeScoringEnabled`/`afRegimeScoringEnabled`) | Splits the scorecard into six segments by which Regime band/side was active at open, instead of one pooled score. See **Regime Based Scoring** under Slot Scorecard below for full mechanics. Only functional once a Regime criterion is selected in MIW/MIC. |
...
The **Combined / Own** toggle (`pfScorecardView`/`afScorecardView`) switches between: Combined (includes partner bot trades, PnL inverted, subject to **Partner Score Push**) sorted by blended total; and Own (this bot's trades only, always unfiltered). Selection is persisted.

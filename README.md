...
| **Depth Multiplier** (`pfCoQualDepthPct`/`afCoQualDepthPct`) | Scales every criterion's collapsed score by how deep the ticker actually qualifies at, both for the best-match slot and any co-qualifying penalty slots. A criterion scoring +$0.50 historically is applied as +$0.70 for a ticker qualifying 4 tiers deep at the default 10%/tier. Works the same in reverse — a losing criterion is penalised harder the deeper the ticker qualifies. Default 10%, range 0–50%. |

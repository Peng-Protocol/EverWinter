...
**80% base-margin floor on absorption cuts:** A cut is skipped if it would leave the position with less than 80% of base margin remaining (base margin = `minNotional / leverage`; at $6 notional and 6× leverage this floor is $0.80). This prevents repeated absorption from reducing margin to a level where the EDa TP recovery target exceeds the entry price — a condition that would silently zero out `_edaTpPrice` and allow the normal 3% reduce TP to fire before accumulated losses are recovered.

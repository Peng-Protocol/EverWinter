...
| **Volume Signal** (`miwFadeVolEnabled`/`micFadeVolEnabled`) | Includes kline volume character in the combined score, gated by scorecard history. Computes whether the current sample leans above-average (lsa/lba territory) or below-average (rasl/rabl territory), then checks whether slots using those criteria have historically lost. If so, the dominant volume direction's share of the sample contributes to the combined score. Off by default. |
| **OI Normalcy Signal** (`miwFadeOiEnabled`/`micFadeOiEnabled`) | Includes aggregate OI/market cap ratio in the combined score. Computed across all tickers with market cap data fresher than 1 hour. Deviation in either direction beyond the tolerance window around the configured midpoint contributes to the adverse score — both over-leveraged and under-leveraged market states are treated as abnormal. Off by default. |
| **OI/MC Midpoint** (`miwFadeOiMid`/`micFadeOiMid`) | Target "normal" OI-to-market-cap ratio in %. Deviation from this in either direction is adverse. Default 2%. Tune after observing live readings — the actual typical ratio for your ticker set may differ. |
| **Tolerance** (`miwFadeOiTol`/`micFadeOiTol`) | How many percentage points of deviation from the midpoint are tolerated before the signal contributes. Beyond this window the score scales proportionally to the excess, capping at 100% at one full tolerance unit of excess. Default ±1. |
| **Structure Weight** (`miwFadeStructureWeight`/`micFadeStructureWeight`) | Controls how much the Structure signal is weighted relative to all other active signals as a group. At 100%: structure carries the same weight as all other active signals combined. At 50% (default): structure contributes half as much as the group. At 0%: structure is excluded from the average. At 200%: structure dominates. Only visible when Structure Signal is on. |
| **Combined Threshold** (`miwFadeThreshold`/`micFadeThreshold`) | % the combined adverse score must reach to trigger a close. Structure is weighted against the mean of all other contributing signals as a group (see Structure Weight); all other signals are averaged equally. Default 50%. |
| **Close Oldest on Adverse Score** (`miwFadeCloseOldest`/`micFadeCloseOldest`) | When combined score ≥ threshold, closes the oldest open MIW/MIC position. Disable to use Fade Away purely for entry blocking without touching open positions. Default off. |
| **Block Entries on Adverse Score** (`miwFadeBlockEntries`/`micFadeBlockEntries`) | Blocks new MIW/MIC entries while the combined adverse score is at or above the threshold. Lifts each scan cycle when conditions clear. |
...
**Fade Away as a drawdown gate replacement**: The consolidated Fade Away system — structure, liq, funding, volume, and OI normalcy signals combined into a single score — can collectively replace drawdown locking for many setups. Close Oldest and Block Entries are independent toggles: you can run both, either, or neither. The combined score reacts to live market conditions; with Close Oldest on it exits the oldest position before losses compound; with Block Entries on it prevents new entries until the environment clears. When slot sorting (Collapsed Sort) is already filtering the entry pool adequately, consider enabling only Block Entries and leaving Close Oldest off — the adverse score then acts as a pause gate rather than an active closer. Liq, Funding, and Volume signals are gated by scorecard history and won't contribute unless the signal has a track record of bad outcomes in your specific slots. OI Normalcy contributes whenever the aggregate market leverage ratio deviates beyond your configured tolerance, with no scorecard gate. Structure Weight lets you amplify or dampen the structure signal's influence relative to the rest — raise it when the tape direction should dominate; lower it when the scorecard-gated signals are considered more reliable. The Loss Trigger enables between-scan checks, so a losing position can be exited before the next scheduled scan without waiting.
...
When **Collapsed Sort** is on, MIW/MIC builds a composite score for each candidate ticker before sorting the entry pool. The composite starts with the ticker's best-matching slot score. If **Co-Qualifying Penalty** is also on, any additional slots the ticker qualifies for are inspected: those with negative collapsed scores subtract from the composite. A ticker that satisfies one strong slot but also satisfies two consistently losing slots ranks lower than a ticker that only satisfies the strong slot. Slots with positive scores do not boost the composite — only negative co-qualifying scores penalise. Tickers are then sorted descending by composite score, so the strongest net-scoring candidates enter first.

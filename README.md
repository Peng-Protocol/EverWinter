...
**liqturnover / liqquiet** (Extremity Scorer dials only — not pickable slot criteria): both sample once per closed liquidation-surveillance batch, market-wide, independent of any individual position. `liqturnover` samples each surveilled symbol's own `max(sDepth, bDepth)` for that batch. `liqquiet` samples `100 − ` the batch's own liquidation-activity % (the share of surveilled symbols that saw ANY liquidation at all this batch, regardless of size) — a high value means most of the market stayed quiet. Both are single-pole families (see **Core idea** above): high and low extremes emerge from where a given batch's reading falls in that stream's own accumulated history, nothing is asserted up front about which direction is "the" extreme.

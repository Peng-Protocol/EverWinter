...
| **Presence Regime Threshold** (`pfPresenceSkewThreshold`/`ashPresenceSkewThreshold`) | Aggregate B-Liq or S-Liq turnover share (50–95%) needed to call the market "dominant" one way or the other. Feeds Speeder's Auto Speed Mode — not scored or traded here directly. |
...
| Setting | What it does |
|---|---|
| **Speeder Mode** (`sdwMode`/`sdcMode`) | **Fast** targets the busy/low-interval half of the order-count sample. **Slow** targets the quiet/high-interval half. **Auto** switches between Fast and Slow using Permafrost/Ashfall's aggregate liquidation regime skew and/or partner-drawdown broadcasts — see Auto Speed below. |

...
**Depth gate** (sliq/bliq/msliq/mbliq): `sliq>N` requires `sDepth ≥ N`; `sliq<N` requires `sDepth ≤ N`. `sDepth`/`bDepth` are computed once per closed liq cycle as `trunc((sideTurnover / openInterestSnapshot) × 100)` — the liquidated side's turnover as a whole percentage of the ticker's open interest at the moment the liq batch launched. `+1` means 1% of open interest got liquidated on that side this cycle; `+5` means 5%, and so on. There's no upper clamp and the value can never go negative (a $/OI ratio can't be negative), so `N` in practice is always 0 or positive — a `<` gate is still meaningful (e.g. `sliq<1` requires under 1% of OI liquidated, i.e. a quiet cycle). Without a suffix the depth gate is skipped. msliq and mbliq use the same depth value as sliq and bliq respectively. Liquid-Diver (below) uses the exact same formula and scale for its own live entry gate — a live reading and a closed-cycle reading mean the same thing.
...
**Depth — read this before setting the threshold.** Liquid-Diver's depth is a live reading of the same formula MultiIndicator's depth gate uses (see above): liquidated $ on the triggering side as a % of the ticker's open interest at batch launch. `+1` means 1% of open interest has been liquidated on that side so far this cycle, `+5` means 5%, and so on — no upper clamp, and the value can never go negative. A live reading and a closed-cycle reading mean exactly the same thing here.

| Setting | What it does |
|---|---|
| **Enabled** (`ldwEnabled`/`ldcEnabled`) | Master on/off. Off by default. |
| **Depth Threshold** (`ldwDepthThreshold`/`ldcDepthThreshold`) | Minimum live depth (% of open interest, see above) to trigger entry. Default 1 (at least 1% of OI liquidated on that side). No ceiling — the 70%/30% liquidation-share requirement already filters out noise. |

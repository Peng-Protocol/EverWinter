...
| 🔭 / 📡 | OCS / OCX. |
| 🌤️ / ☁️ / 🌪️ | Regime Lo / Regime Mid / Regime Hi; side is encoded by the criterion name (`regimes*` or `regimeb*`). |
...
**Regime** (`regimeslo`/`regimeblo`/`regimesmid`/`regimebmid`/`regimeshi`/`regimebhi`): reads Permafrost/Ashfall's population-wide liquidation skew (`_pfRegimeSkew`/`_afRegimeSkew`) rather than anything ticker-specific — the same reading for every candidate ticker in a given cycle, recomputed on demand from retained cycle history with no prefetch or scan cap. Band thresholds are set under **Regime Mid/Hi Threshold** in Config below; the Lo floor is fixed at 50 (the mathematical minimum a dominant side can read) and isn't configurable. All six are mutually exclusive — exactly one is true whenever any turnover has been recorded, never more than one at once. The tag's numeric suffix is the dominant share's raw percentage-point distance above its own band's floor, not always measured from 50 — `regimeslo` at 59% records `regimeslo+9` (59 − 50); `regimeshi` at 81% records `regimeshi+1` (81 − 80, the Hi floor). Selecting at least one Regime criterion here also turns on **Regime Based Scoring** in Permafrost/Ashfall — see that section under Extremity Scorer for the six-segment scoring split this enables.
...
| **Regime Mid Threshold** (`miwRegimeMidThreshold`/`micRegimeMidThreshold`) | Dominant-side turnover share (default 60, range 55–90) where Regime Mid starts. Below this is Regime Lo (fixed floor at 50, not configurable). |
| **Regime Hi Threshold** (`miwRegimeHiThreshold`/`micRegimeHiThreshold`) | Dominant-side turnover share (default 80, range 60–95) where Regime Hi starts. Below this is Regime Mid. |
...
- Regime is exclusive across all six, not just within a pair — since the dominant side and its band are read from one population-wide number each cycle, at most one of `regimeslo`/`regimeblo`/`regimesmid`/`regimebmid`/`regimeshi`/`regimebhi` can ever be true at once.
...
| **Regime Based Scoring** (`pfRegimeScoringEnabled`/`afRegimeScoringEnabled`) | Splits the scorecard into six segments by which Regime band/side was active at open, instead of one pooled score. See **Regime Based Scoring** under Extremity Scorer below for full mechanics. Only functional once a Regime criterion is selected in MIW/MIC. |
| **Regime Min Sample** (`pfRegimeMinSample`/`afRegimeMinSample`) | Minimum closes a criterion needs within the live regime segment before Slot Blocking/Prospect Pre-Screening/Substitution trust that segment's figure over the flat aggregate. Default 5, range 1–30. Display always shows the segment's raw figure regardless of this setting — only live blocking/ranking falls back below it. |
...
| **Stale Slot Purge** (`pfStaleSlotDays`/`afStaleSlotDays`) | Slots with no new close in this many days are fully removed from the scorecard on the next trim. Keeps the scorecard clean when criteria combinations fall out of use. Plain number field, no upper bound. Freshness is per exact criteria combo — except while **Regime Based Scoring** is on, when it's shared per individual criterion instead; see below. |
...
**Core idea**: rather than just pooling raw PnL per criterion, continuous/tiered criteria are grouped into **dial families** — `fund`, `va`, `ioa`, `ocs`, `ocx`, `pec`, `vsg`, `regime` — and every recorded close's magnitude is classified as **extreme** or **lukewarm** against a cutoff derived from that family's own sample (top **Extreme Split**% by magnitude, once at least **Min Samples** closes exist — below that, the reading is "unproven" and counts as lukewarm). Two-pole families pit their positive and negative sides against each other, each pole's cutoff computed independently from its own sample; the single-pole Irregularity readings (`pecirr`/`vsgirr`) only classify hot-vs-lukewarm within their own spread, with no opposing side. Liq anchors (`sliq`/`bliq`/`msliq`/`mbliq`/`0liq`) sit outside the dial system entirely — they stay binary win/loss.
...
**Extremity Re-Evaluation** (off by default) applies this same veto over a position's *whole life*, not just at entry — every scan cycle, each open position's live criteria are re-derived against the current dial cutoffs, and a position that's fallen back to lukewarm is closed and tagged **`Ext-Rvl`** in the trade log.

**Pill display (Chaser/Winter View)** — one split pill per dial family: a positive-pole half and a negative-pole half side by side with a thin divider between them, each half showing its own emoji, PnL, W/L record, and its own extreme cutoff independently — e.g. `fund`'s positive half might read `›0.2` while its negative half reads `‹-0.8`; the two poles' cutoffs are computed from separate samples and aren't mirrored copies of each other. Regime's bull/bear sides fold into one pill the same way `fund`/`pec`/`vsg` do, rather than showing as two separate one-ended cards. Liq anchors and the single-pole Irregularity readings render as a one-ended pill (no opposite pole exists for either). Only shown when MIW/MIC is both loaded and enabled. **Purple PnL text** marks a criterion currently contributing to a Slot Blocking pause — hover a pill for the exact reason.

**CLEAR** wipes the record store. Each close writes one record; the Sponge Quota controls how many recent records per criterion feed both the collapsed PnL score and the dial cutoff, so both adapt to the most recent N closes. Cutoffs (and every dial-based score with them) are rebuilt once per scan cycle rather than only on a close/config-change/import — the underlying sample pool is deliberately not persisted across page reloads, so without this a fresh page load would show every dial-based score as flat zero until a real trade happened to close, sometimes hours later. MIW/MIC uses its own chart's per-criterion scores to order its entry queue — slots are ranked by the sum of their individual criteria scores. A slot with one losing criterion and one winning criterion ranks by their combined total.
...
**Regime Based Scoring** (`pfRegimeScoringEnabled`/`afRegimeScoringEnabled`) splits the same criteria into six segments — one per Regime band/side (`regimeslo`/`regimeblo`/`regimesmid`/`regimebmid`/`regimeshi`/`regimebhi`, see **Regime** under MultiIndicator Plugin) — instead of one pooled score across all history. A record only lands in a segment if it carried a matching Regime tag at open, which itself only happens when a Regime criterion is selected in MIW/MIC's slot config; records with no Regime tag are simply excluded from every segment, not folded into a catch-all. Only shown, and only functional, once at least one Regime criterion is currently selected — the toggle has no effect otherwise.

- **Segment display** replaces the ordinary flat pill list when active. A cycle button at the bottom-left of the chart (labeled Low/Mid/High Bull/Bear — Bull = S-Liq dominant, Bear = B-Liq dominant) switches which segment is shown; it only lists segments whose Regime criterion is currently selected, and cycles among those. With nothing manually picked, the display **auto-follows the live regime** — but only re-picks a segment on an actual Liq-skew threshold cross or an app reload, checked every 15s, not on every render, so it doesn't jitter if skew is sitting right at a band edge. Purple blocked-highlighting on a pill reflects that *specific segment's* own score, not the live regime — browsing a segment you're not currently in shows whether that segment's own history would block the criterion, not whether it's blocked right now.
- **Live blocking and entry ordering** (Slot Blocking, Prospect Pre-Screening, Co-Qualifying Penalty, Substitution) follow the *currently prevailing* regime's segment automatically, independent of whatever segment is being browsed — MIW/MIC's own scoring functions (`_miwCritScore`/`_micCritScore`, `_miwScaledRank`/`_micScaledRank`) read the live-regime figures directly rather than the flat aggregate whenever this toggle is on. Below **Regime Min Sample** (`pfRegimeMinSample`/`afRegimeMinSample`, default 5) closes for a given criterion within the live segment, that criterion falls back to the flat aggregate for blocking/ranking purposes only — a thin regime bucket shouldn't drive real entry decisions, even though the display still shows that segment's raw (unpadded) figures.
- **Stale Slot Purge** changes behavior while this is on: splitting every combo six ways means the same real trading frequency is spread across six buckets, so purging by each bucket's own last-write time would purge far more aggressively than before. With Regime Based Scoring on, freshness is instead shared per individual criterion — any record touching a criterion refreshes every combo containing it, not just that exact combo — so a combo doesn't go stale just because *that specific* combination hasn't recorded a close recently while its component criteria have. Off, purge behavior is unchanged (per-combo freshness).

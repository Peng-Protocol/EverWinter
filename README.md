...
| **Gains Continuation (GC)** (`pfGcEnabled`/`afGcEnabled`) | Independent of the halt system's own Gains Lock: tracks this bot's own rolling 6h closed-trade PnL and broadcasts a one-shot `gc` event each time it crosses a new step (`pfGcThresholdPct`/`afGcThresholdPct`, default 50% of base margin — high-water-mark, a step only re-fires after the window resets to ≤0 and climbs again). If the partner also has GC on and reads a step within the freshness window (`pfGcFreshnessMult`/`afGcFreshnessMult` × Scan Interval, default 2×), it triggers a *full drawdown halt on itself* — a winning streak on one side is read as a signal for the other to step back rather than chase the same conditions. Off by default; requires GC on both bots and Cross Broadcast on both bots. |

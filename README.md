...
**Depth recording**: the tag's numeric suffix is the dominant share's raw percentage-point distance *above its own band's floor* — not always measured from 50. `regimeslo` at 59% records `regimeslo+9` (59 − 50); `regimeshi` at 81% records `regimeshi+1` (81 − 80, the Hi floor), not `regimeshi+31`. The suffix is always non-negative and always uses `+` — the criterion's own name (`regimes*`/`regimeb*`) already encodes side, so the depth needs no separate sign. (Earlier builds tied the sign to whichever side currently dominated the market rather than the band itself, which could leave a stray `-` behind once `_miwCritScore`/`_micCritScore` stripped the trailing digits — surfacing as a phantom `regimebmid-`-style entry in the scorecard distinct from the real `regimebmid`. Fixed.) No step/tier size setting exists for this — unlike PEC/VSG Depth Step, the raw point offset is recorded directly. This only affects the displayed tag and scorecard log entries; `_miwCritScore`/`_micCritScore` collapse every tag's trailing digits before scoring, so the exact offset never changes a blocking or scoring outcome, only what the log shows.

Selecting at least one Regime criterion here is also what turns on **Regime Based Scoring** in Permafrost/Ashfall — see that section under Slot Scorecard for the six-segment scoring split this enables.
...
| **Protect Winners** (`miwSubstitutionProtectWinners`/`micSubstitutionProtectWinners`) | Off by default. When on, a position currently in profit is never substituted out, regardless of how it ranks. Turn on if temporary winners in your markets tend to hold rather than reverse quickly, and should be shielded just for being ahead right now. |
...
| `__everwinter_cross_v1` | Cross-bot event log written by Permafrost/Ashfall's **Cross Broadcast** — cascade/sacrifice closes, drawdown halt/gains-lock start/end transitions, live liquidation events, and 0-Liq wins, each tagged with its source bot. Read every 15 s by the partner for Mutual DDH Lift's age comparison and the recent-events display. Retained 25h (must outlast the longest configurable halt/gains-lock duration, 24h). |

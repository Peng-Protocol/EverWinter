...
Every other criterion reads something about the specific ticker under consideration. Regime doesn't — it reads Permafrost/Ashfall's own population-wide liquidation skew (`_pfRegimeSkew`/`_afRegimeSkew`, the same aggregate the **Presence Regime** setting has driven since before MIW/MIC consumed it — see Permafrost/Ashfall Config below), the same number for every candidate ticker in a given cycle. It's the first criteria group not derived from the ticker being evaluated.
...
**No fetch, no scan-cap scope**: `_pfRegimeSkew`/`_afRegimeSkew` already exist, recomputed on demand from `_liqCycleHistory` for Presence Regime. Regime's checker reads that directly — no prefetch, no liq-fresh pool restriction, no PEC/VSG-style scan cap. This is deliberate: PEC/VSG's Universe Weighing prefetch is what caused Pre-Screening degradation when it was first wired in (see Changelog) — Regime carries none of that cost by construction.
...
| **Presence Regime Threshold** (`pfPresenceSkewThreshold`/`ashPresenceSkewThreshold`) | Aggregate B-Liq or S-Liq turnover share (50–95%) needed to call the market "dominant" one way or the other for `_pfIsBliqDominant`/`_pfIsSliqDominant`. Not scored or traded here directly, and still has no active consumer of its own — MIW/MIC's new **Regime** criteria read the same underlying skew but band it against their own separate Mid/Hi Threshold config, not this one. |
| **Presence Regime Window** (`pfPresenceSkewWindow`/`ashPresenceSkewWindow`) | How many of the most recently closed liq cycles to aggregate for the regime read (0–25). 0 uses the entire retained history. Now has an active consumer: MIW/MIC's **Regime** criteria read `_pfRegimeSkew`/`_afRegimeSkew` directly, which aggregates over exactly this window — narrowing or widening it changes how quickly Regime's band/direction read reacts to a shift in liquidation flow. |
| **When Regime Is Mixed** (`pfPresenceMixedMode`/`ashPresenceMixedMode`) | Previously governed what Auto-mode consumers (MIW/MIC, Speeder) did when neither side crossed the Presence Regime Threshold. No active consumer now that both have been replaced by Slot Blocking — and MIW/MIC's Regime criteria don't have a mixed case to consume either, since the dominant side is always ≥50% by construction. |

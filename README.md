...
**Order Interval chart note**: the ORDER INTERVAL SAMPLE chart in Permafrost/Ashfall hides itself when Liquid-Diver is the only strategy plugin loaded. Liquid-Diver's OC fetch is single-symbol per reactive entry, so its samples are almost always a single flat bar — showing that chart alongside a proper batch-sampled chart (from MultiIndicator or Blind-Entry) would just be noise. Load MultiIndicator or Blind-Entry alongside Liquid-Diver to see the chart again.

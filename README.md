...
| **OI/MC Midpoint** (`miwFadeOiMid`/`micFadeOiMid`) | Target "normal" OI-to-market-cap ratio in %. Deviation from this in either direction is adverse. Default 10%. Tune after observing live readings — the actual typical ratio for your ticker set may differ. |
| **Tolerance** (`miwFadeOiTol`/`micFadeOiTol`) | How many percentage points of deviation from the midpoint are tolerated before the signal contributes. Beyond this window the score scales proportionally to the excess, capping at 100% at one full tolerance unit of excess. Default ±10. |

...
| **Auto Presence — Base** (`miwPresenceBase`/`micPresenceBase`) | Only shown when Liquidation Presence is Auto. Four options: **Only-1 🥵^**, **Only-1 🥶^**, **Only-0 🥶^**, **Only-0 🥵^** (🥵 = S-Liq, 🥶 = B-Liq, `^` marks the anchor). Each names which mode runs when that regime is dominant — the opposite regime always gets the opposite mode, so picking one option fully determines both. E.g. **Only-1 🥵^** means Only 1 while S-Liq is dominant, Only 0 while B-Liq is dominant. |
| **Auto Presence — Trigger** (`miwPresenceAutoTrigger`/`micPresenceAutoTrigger`) | Only shown when Liquidation Presence is Auto. **Only-Skew** flips per Base's anchor whenever Permafrost/Ashfall's aggregate B-Liq/S-Liq turnover crosses their Presence Regime Threshold. **Only-Drawdown** flips to the opposite mode only while the partner bot is in a drawdown halt (requires Cross Broadcast). **Both** flips on either signal. When neither regime is dominant (mixed) and no drawdown flip applies, Permafrost/Ashfall's "When Regime Is Mixed" setting decides whether this stays on Base's own mode or halts new entries until the regime turns directional again — existing positions are unaffected either way, and Historical Scoring keeps logging through the halt the same as a drawdown halt. |

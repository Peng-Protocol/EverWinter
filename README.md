...
- **Past-1** — **not a criterion**. A Past-1 slot has no liquidation criterion in it at all; it's built entirely from secondary criteria. What makes it a Past-1 slot is that it's only ever matched against tickers with real recent liquidation activity that never qualified for Live (didn't cross the live threshold) or 0-Liq (wasn't clean) — see `_miwPast1FreshSymbols`/`_micPast1FreshSymbols`. The ticker-pool restriction *is* the anchor; there's nothing to tag in the slot or the scorecard.
...
See Strategy_book.md's **Market Reading** section for what each criterion (`fund`, PEC, `vm`, `lsa`/`lba`/`rasl`/`rabl`, `iot`/`iom`, `sliq`/`bliq`/`msliq`/`mbliq`, `0liq`, Buy/Sell Skew, Order Count Deviation) means and when to use it. The mechanical notes below cover tiering, recording, and the depth gate — operational detail Strategy_book intentionally leaves out.

**Tier gate**: all tiered criteria (`fund`, `vm`, `lsa`, `lba`, `rasl`, `rabl`, `iot`, `iom`, `ocs`, `ocx`) compute an integer tier as `floor(value / step)` and compare it against N. Current step sizes are shown as a read-only info line under **Tier Step Sizes** in the Entry Zone. The same tier is recorded on the position and used as the scorecard key — `lsa>25` and `lsa+37` are different tiers and scored separately. PEC criteria (below) are matched differently — fixed thresholds, not a tier comparison — but still record a depth-based tier for scorecard purposes.

### PEC (Price Effect Coefficient)

PEC replaces the old raw `+24h`/`-24h` ticker-field boolean with a graded read of where price has actually been across three timeframes — 6h, 12h, and 24h — each scored 0–100 (50 = neutral) from a composite candle built out of cached klines. Eight criteria, all derived from the resulting 6h/12h/24h triple:
...
| `pecbull` (Bullish Reversion) | 24h > 50 (up on the day) but 6h < 50 (pulled back short-term) |
| `pecbear` (Bearish Reversion) | 24h < 50 (down on the day) but 6h > 50 (bounced short-term) |
...
| **V/M Scan Cap** (`miwMcapScanCap`/`micMcapScanCap`) | Max tickers for which CoinGecko market cap is fetched per scan cycle. Tickers are pre-screened by fund rate before the cap is applied, so only genuine candidates count against it. Bounding this keeps the CG request small and prevents failed fetches from leaving stale market cap data in the pool. |
...
| **Include Sign Tags** (`miwSlotBlockTags`/`micSlotBlockTags`) | Sub-toggle under Slot Blocking. When on, a losing kline (`lsa`/`lba`/`rasl`/`rabl`) or PEC score also blocks entries carrying that tag, on top of the liq-type check above. Deliberately scoped to just these two groups — `fund`, `vm`, `iot`/`iom`, and `ocs`/`ocx` are excluded because they're too generalized: most entries across every slot share the same fund tier or a similar V/M/OI band regardless of outcome, so blocking on them would end up pausing nearly the entire book off one bad stretch that had nothing specifically to do with that shared reading. Off by default. |
| **Live Depth Threshold** (`miwLiveDepthThreshold`/`micLiveDepthThreshold`) | General live depth threshold (% vs. historical average liquidation size) applied to every Auto-Slot live liq anchor. Only shown when Auto Slots is on and the Live presence mode is active — manual slots set depth per-slot instead, via the criteria picker's depth suffix. This is also the threshold Past-1 checks retrospectively against each closed batch's dominant side, to tell "would have fired live" apart from "didn't beat the threshold." |
...
| **Self-Liq Graylist** (`miwSelfLiqGraylistEnabled`/`micSelfLiqGraylistEnabled`) | When on, any liquidation this bot's own surveillance records for a ticker — even one that never crossed the qualifying % threshold — graylists that ticker from the 0-Liq path for the last 25 liq batches. Redundant with 0-Liq Lookback above (which already checks the same retained window directly) but left on as a backstop. Only has an effect while '0' is an active Liquidation Presence mode — with only Live/Past-1 active, there's no 0-Liq path to guard. |
...
| **Auto Slots** (`miwAutoSlots`/`micAutoSlots`) | Replaces the manual slot list with every possible combination of the available criteria at the chosen size. Combinations for Live/0-Liq anchor on a liquidation criterion (filtered by Liquidation Presence); if Past-1 is an active mode, combinations built entirely from secondary criteria — no liquidation criterion at all — are also generated, since Past-1's anchor is the ticker pool it's matched against, not a criterion in the slot. |
| **Auto Slot Size** (`miwAutoSlotSize`/`micAutoSlotSize`) | How many criteria per auto-generated combination (1–4). For Live/0-Liq combinations this counts the mandatory liq anchor as one of them — at size 1, every Live/0-Liq slot is bare liquidation with no secondary confirmation. Past-1 combinations use the full size in secondary criteria instead, since there's no anchor slot to reserve room for. |
| **Exclude from Auto** (`miwAutoSlotExclude`/`micAutoSlotExclude`) | Criteria omitted from auto-generated combinations and from scorecard scoring. Excluding all liq types allowed under the current Liquidation Presence mode leaves nothing for Live/0-Liq to anchor on; Past-1 combinations still generate normally as long as Past-1 is an active mode, since they don't need one. |
...
**Auto mode**: replaces the manual builder with a size picker and an exclusion chip row. Live/0-Liq combinations anchor on one liquidation criterion (the types allowed depend on the Liquidation Presence setting) plus secondary criteria filling the remaining slots; if Past-1 is active, secondary-only combinations (no anchor, full size) are generated alongside them. The combo count shown reflects both.

- Many generated combinations are contradictory and can never match any ticker: a slot containing both `pecob` and `pecos` (or any of the other three PEC opposite pairs — `pecbull`/`pecbear`, `pecalpha`/`pecdelta`, `pecmomup`/`pecmomdn`) can never be satisfied simultaneously, nor can any two from the candle group (`lsa`, `lba`, `rasl`, `rabl`), since a candle only closes in one direction with one volume character at a time. At secondary-size 2 these pairs are common, so the effective slot count is lower than the number shown. Excluding one side of each pair removes the dead combinations.
...
| **React to Partner Live-Liq** (`pfCrossLiqGraylistEnabled`/`afCrossLiqGraylistEnabled`) | When the partner bot logs a live liquidation on a ticker, graylists that ticker from this bot's own 0-Liq path for 6 hours. Only has an effect while '0' is an active Liquidation Presence mode on this bot — with only Live/Past-1 active, there's no 0-Liq path to protect. |
...
| `__ew_shared_v1` | Cross-tab shared data registry (bulkTickers, watchTickers, klines_1h, klines_1h_last, pec_{granularity}_last, mcap, liqResults) |

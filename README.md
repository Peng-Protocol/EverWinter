...
| **Liquidation Presence** (`miwLiqPresenceMode`/`micLiqPresenceMode`) | Three-way toggle: **Only 1** (default) runs only the live sliq/bliq/msliq/mbliq path. **Only 0** runs only the closed-cycle 0-Liq path. **Both** runs each independently, producing their own slots. Requires Liquidation Surveillance in Permafrost/Ashfall — without it there's no liq data at all and no slot can ever match. |
...
| **Re-entry Block Mode** (`miwReentryBlockMode`/`micReentryBlockMode`) | **Winners + Losers** (default) blocks a symbol from re-triggering for one bulk-ticker cooldown window after any close, win or loss. **Losers Only** lets a symbol that just closed in profit re-trigger immediately, blocking only a symbol that closed at a loss. |
| **Self-Liq Graylist** (`miwSelfLiqGraylistEnabled`/`micSelfLiqGraylistEnabled`) | Off by default. When on, any liquidation this bot's own surveillance records for a ticker — even one that never crossed the qualifying % threshold — graylists that ticker from the 0-Liq path for 6 hours. Guards against a ticker that liquidated a couple hours ago looking freshly "calm" just because its most recent surveillance cycle happened to close clean. |
...
| **React to Partner Live-Liq** (`pfCrossLiqGraylistEnabled`/`afCrossLiqGraylistEnabled`) | Off by default. When the partner bot logs a live liquidation on a ticker, graylists that ticker from this bot's own 0-Liq path for 6 hours. Only has an effect while this bot's Liquidation Presence isn't Only-1 — a bot running Only-1 has no 0-Liq path to protect. |
| **React to Partner 0-Liq Win** (`pfCrossWinGraylistEnabled`/`afCrossWinGraylistEnabled`) | Off by default. When the partner bot wins a 0-Liq trade on a ticker, graylists that ticker from this bot's own live liq entries for 6 hours. Only has an effect while this bot's Liquidation Presence isn't Only-0 — a bot running Only-0 has no live path to protect. |
...
| **OI/MC History** | A line chart showing aggregate OI as a % of market cap over time, sampled from the bulk ticker feed at the same cadence as structure wave — independent of which MIW/MIC entry criteria happen to be active, so it keeps updating even when no active slot uses `vm`/`iom`. Market caps behind the ratio are kept fresh by a small top-up fetch whenever a liquidation surveillance batch launches. Current ratio shown next to the label. Only appears when the OI fade signal is enabled. |
...
**Requires MultiIndicator**: the Funding Rate Sample, Volume Sample, Order Count Surveillance, and the Slot Scorecard are all computed by MIW/MIC's own sampling pass and disappear entirely if MultiIndicator isn't loaded — they are not shown empty, they don't render at all. OI/MC History is sampled by Permafrost/Ashfall itself rather than MIW/MIC's sampling pass, but still relies on MultiIndicator's market-cap fetching, so it disappears the same way if MultiIndicator isn't loaded.

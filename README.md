...
**Tier gate**: all tiered criteria (`fund`, `vm`, `lsa`, `lba`, `rasl`, `rabl`, `iot`, `iom`, `ocs`, `ocx`) compute an integer tier as `floor(value / step)` and compare it against N. Current step sizes are shown as read-only chips under **Tier Step Sizes** on the Fetch tab. The same tier is recorded on the position and used as the scorecard key — `lsa>25` and `lsa+37` are different tiers and scored separately. PEC criteria (below) are matched differently — fixed thresholds, not a tier comparison — but still record a depth-based tier for scorecard purposes.
...
| **Tier Step Sizes** (`miwFundStep`/`miwVmStep`/`miwSpikeStep`/`miwIotStep`/`miwIomStep`/`miwOcsStep`/`miwOcxStep` and `mic` equivalents) | Read-only chip group on the **Fetch** tab showing the current tier step size for `fund`, `vm`, `lsa`/`lba`/`rasl`/`rabl` (spike), `iot`, `iom`, `ocs`, and `ocx`. `ocx`'s step is a percentage-deviation-from-population-average step (default 1%), not seconds — see **OCS/OCX recording** above. Not editable from the UI — change via config import if a different step is needed. |
...
| **Blockable Criteria** (`miwSlotBlockCriteria`/`micSlotBlockCriteria`) | Chip picker, same interaction as **Exclude from Auto**, listing every secondary criterion as toggleable. Only shown when Include Sign Tags is on. Defaults to `lsa`/`lba`/`rasl`/`rabl`/all six PEC criteria — the same fixed set this used to be hardcoded to — preserved as the default so nothing changes on upgrade. `fund`, `vm`, `iot`/`iom`, and `ocs`/`ocx` are available but off by default: they're more generalized, since most entries across every slot share a similar fund tier or V/M/OI/order-flow band regardless of outcome, so blocking on them risks pausing nearly the entire book off one bad stretch that had little to do with that specific reading. Enable with care. |
...
| **Force-Evaluate General Criteria** (`miwForceGeneralCriteria`/`micForceGeneralCriteria`) | Auto mode only. `fund`, PEC, `ocs`/`ocx`, and `iom` are expected to resolve for virtually any ticker once real data exists, but a match can end up missing several of them purely because that cycle's capped bulk fetch hadn't covered the ticker yet — not because they're actually false. When on: the live entry path force-fetches PEC on-demand for the entering symbol (mcap/kline/OC already did this unconditionally; PEC previously didn't); the scan-time open path (`_miwOpen`/`_micOpen`, via `_miwTopUpGeneralCriteria`/`_micTopUpGeneralCriteria`) re-checks `fund`/PEC/`ocs`/`ocx`/`iom` with a forced, uncapped, per-symbol fetch — only for the handful of tickers actually about to open, never the whole pool — and appends any newly-true criteria, then recomputes compositeScore over the expanded set. Never drops a criterion the original match already found. `iot` needs no fetch and is unaffected — it's derived directly from the ticker snapshot, so it was never missing for a data-availability reason. Costs a few extra requests per entry. |
...
- Some criteria can never both be true for the same ticker at the same time: the three PEC opposite pairs (`pecob`/`pecos`, `pecalpha`/`pecdelta`, `pecmomup`/`pecmomdn`) and any two from the candle group (`lsa`, `lba`, `rasl`, `rabl`) — a candle only closes in one direction with one volume character at once. This doesn't need excluding anything to avoid dead combinations the way the old fixed-size generator did; a ticker simply can't ever contribute both sides toward its qualifying count.
- `lsa`/`lba`/`rasl`/`rabl` almost never co-qualify with `vm` criteria for the same ticker: `vm` criteria apply only to a capped subset of top tickers by turnover (V/M Scan Cap), while kline criteria sample randomly from the full pool — the overlap between those two subsets is small in practice.

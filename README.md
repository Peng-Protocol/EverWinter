...
| **Graylist Cooldown** (`pfBlockedSlotCooldownHours`/`afBlockedSlotCooldownHours`) | How long a ticker matching a blocked slot stays graylisted (0.5–48h). Controls the cooldown for all three blocking paths (Auto Block, Collapsed Scoring, Live Collapsed Gate). Default 6h. |
...
The **Both / Hist / Traded** source filter (`pfScorecardSource`/`afScorecardSource`) cycles through which score records are included in the displayed chips. **Both** (default) shows all records. **Hist** shows only records written by the MIW/MIC Historical Scoring feature. **Traded** shows only records from actual live closed trades. This is a display-only filter — it does not affect which records count toward auto-block decisions or collapsed scoring. Selection is persisted.
...
| **Collapsed Scoring** | Each scorecard rebuild | Annotated slot key (criteria + tier suffixes) aggregate score — collapsed PnL × tier-depth scale factor — falls below Collapsed Threshold | Slot key graylisted for the Graylist Cooldown duration; tickers matching that exact key at scan time are skipped | Next rebuild if the score recovers above threshold |
| **Live Collapsed Gate** | Per ticker, per scan | Same scoring formula as Collapsed Scoring but computed using the ticker's actual live criterion depths at the moment of the scan | Ticker graylisted for the Graylist Cooldown duration | Cooldown expiry or page reload |

...
| **Auto Presence — Base** (`miwPresenceBase`/`micPresenceBase`) | Only shown when Liquidation Presence is Auto. The resting mode (Only 1 or Only 0) used whenever the regime isn't flipped away from it. |
| **Auto Presence — Trigger** (`miwPresenceAutoTrigger`/`micPresenceAutoTrigger`) | Only shown when Liquidation Presence is Auto. **Only-Skew** flips off Base when Permafrost/Ashfall's aggregate B-Liq turnover share crosses their Presence Regime Threshold — B-Liq dominant flips to the opposite of Base, S-Liq dominant stays on Base. **Only-Drawdown** flips off Base only while the partner bot is in a drawdown halt (requires Cross Broadcast). **Both** flips if either signal fires. When neither side is dominant (a mixed regime) and no drawdown signal applies, Permafrost/Ashfall's "When Regime Is Mixed" setting decides whether this stays on Base or halts new entries until the regime turns directional again — existing positions are unaffected either way, and Historical Scoring keeps logging through the halt the same as a drawdown halt. |

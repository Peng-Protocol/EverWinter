...
| `pecirr` (Irregular) | Average price efficiency across all three windows falls below **Irregularity Threshold** (default 60) — the tape is spikey/choppy rather than directional |

`pecalpha`/`pecdelta`/momentum only compare the three readings to each other — none of the six pivot on a fixed midpoint the way the old removed `pecbull`/`pecbear` reversion pair did. Only `pecob`/`pecos` gate on Threshold High/Low. `pecirr` is the exception among the seven — it doesn't compare timeframes to each other at all, it measures how directional each window's own price path was.
...
**Irregularity and momentum**: `pecirr` measures "spikiness" — an efficiency-ratio read comparing how far apart a window's visited price extremes actually are (net range) against how much ground was covered getting there (summed per-candle wick travel). A directional tape mostly extends its own extremes with each candle, so net range tracks travel closely (low irregularity); a choppy tape revisits the same band repeatedly, piling up travel far past net range (high irregularity). This reuses the exact same high/low data Wick Dampening already computes — no new fetch. Once a window's average irregularity clears **Irregularity Threshold**, `pecmomup`/`pecmomdn`'s strict 6h>12h>24h ordering is no longer treated as believable momentum on its own — a monotonic sequence in a genuinely choppy window is as likely to be coincidence as trend. Above the threshold, momentum instead falls into the same nearest-equivalent tie-break scoring described above, with its score linearly dampened toward zero as irregularity climbs from the threshold toward 100 — `pecalpha`/`pecdelta` are unaffected either way.
...
| `vsgirr` (Irregular) | Average volume efficiency across all three windows falls below **Irregularity Threshold** (default 60) — activity is spikey/erratic rather than trending toward one extreme |

`vsgalpha`/`vsgdelta`/momentum only compare the three readings to each other, same as PEC's equivalent quartet; only `vsgot`/`vsgut` gate on Threshold High/Low. Same tie-break fallback as PEC's shape quartet (`_miwVsgShapeMatch`/`_micVsgShapeMatch`) for a flat-range window that reads exactly 50 on all three. `vsgirr` doesn't participate in that tie-break — like `pecirr`, it measures each window's own volume path rather than comparing timeframes to each other.

**Irregularity and momentum**: `vsgirr` is `pecirr`'s volume-side twin — same efficiency-ratio idea, but since volume has no per-candle high/low of its own, "travel" is the sum of absolute candle-to-candle volume swings across the window, compared against VSG's own vHigh−vLow net range. A volume series trending steadily toward one extreme keeps travel close to net range (low irregularity); one that spikes up and down between similar levels repeatedly piles up travel far past net range (high irregularity). Same threshold, same momentum-dampening treatment as PEC's `pecmomup`/`pecmomdn` above — above **Irregularity Threshold**, `vsgmomup`/`vsgmomdn`'s strict ordering stops auto-qualifying and falls into the dampened tie-break instead; `vsgalpha`/`vsgdelta` are unaffected.
...
Every other criterion reads something about the specific ticker under consideration. Regime doesn't — it reads Permafrost/Ashfall's population-wide liquidation skew (`_pfRegimeSkew`/`_afRegimeSkew`), the same number for every candidate ticker in a given cycle. It's the first criteria group not derived from the ticker being evaluated.

Six criteria, one per band/side combination:

| Criterion | Fires when |
|---|---|
| `regimeslo` (Regime S-Lo) | Sell-liq turnover share of the aggregate is dominant and sits 50–59% |
| `regimeblo` (Regime B-Lo) | Buy-liq turnover share is dominant and sits 50–59% |
| `regimesmid` (Regime S-Mid) | Sell-liq dominant, 60–79% |
| `regimebmid` (Regime B-Mid) | Buy-liq dominant, 60–79% |
| `regimeshi` (Regime S-Hi) | Sell-liq dominant, 80%+ |
| `regimebhi` (Regime B-Hi) | Buy-liq dominant, 80%+ |

Since the two sides' shares sum to 100%, the dominant side is mathematically always ≥50% — exactly one of the six is true whenever any turnover has been recorded at all, same always-resolvable property as PEC's/VSG's shape quartet. All six are mutually exclusive with each other, not just within a band — unlike PEC/VSG, which have three independent opposite pairs and can co-fire across pairs (`pecob` and `pecmomup` at once, for instance), Regime can never contribute more than one criterion to the same match.

**No fetch, no scan-cap scope**: `_pfRegimeSkew`/`_afRegimeSkew` are recomputed on demand from the retained `_liqCycleHistory`. Regime's checker reads that directly — no prefetch, no liq-fresh pool restriction, no PEC/VSG-style scan cap. This is deliberate: PEC/VSG's Universe Weighing prefetch is what caused Pre-Screening degradation when it was first wired in (see Changelog) — Regime carries none of that cost by construction.

**Band thresholds**: `miwRegimeMidThreshold`/`micRegimeMidThreshold` (default 60, range 55–90) and `miwRegimeHiThreshold`/`micRegimeHiThreshold` (default 80, range 60–95) set the Mid and Hi floors. The Lo floor is fixed at 50 — the mathematical minimum a dominant side can read — and isn't configurable.

**Depth recording**: the tag's numeric suffix is the dominant share's raw percentage-point distance *above its own band's floor* — not always measured from 50. `regimeslo` at 59% records `regimeslo+9` (59 − 50); `regimeshi` at 81% records `regimeshi+1` (81 − 80, the Hi floor), not `regimeshi+31`. The sign carries direction rather than the band: sell-dominant is always positive, buy-dominant is always negative, regardless of which of the three bands fired — `regimeblo` at 51% records `regimeblo-1`. No step/tier size setting exists for this — unlike PEC/VSG Depth Step, the raw point offset is recorded directly. This only affects the displayed tag and scorecard log entries; `_miwCritScore`/`_micCritScore` collapse every tag's trailing digits before scoring, so the exact offset never changes a blocking or scoring outcome, only what the log shows.
...
| **Blockable Criteria** (`miwSlotBlockCriteria`/`micSlotBlockCriteria`) | Chip picker, same interaction as **Exclude from Auto**, listing every secondary criterion as toggleable. Only shown when Include Sign Tags is on. Defaults to all seven VSG criteria (including `vsgirr`)/all seven PEC criteria (including `pecirr`)/all six Regime criteria/`va`/`ioa`. `va`/`ioa` are on by default — being population-relative rather than an own-ratio, a consistently losing deviation from the crowd is a meaningful signal worth pausing on. Regime is on by default too, for a related but distinct reason: it's population-wide rather than ticker-derived at all, so a losing band/direction reading is a statement about market conditions themselves, not about any one ticker's behavior. `fund` and `ocs`/`ocx` remain off by default: they're more generalized, so blocking on them risks pausing nearly the entire book off one bad stretch that had little to do with that specific reading. Enable with care. `fund`'s bare (un-thresholded, Auto-mode) form now requires the funding rate actually clear one full **Fund Step** off zero to count as true at all — it used to count unconditionally regardless of tier, which meant Auto mode tagged virtually every match with a `fund+`/`fund-` reading whether or not funding had anything to do with it. |
...
| **Cross Broadcast** (`permafrostCrossEnabled`/`ashfallCrossEnabled`) | Writes cascade/sacrifice closes, drawdown halt/gains-lock transitions, live liquidation events, and 0-Liq wins to a shared log that the partner bot can read. Required for Mutual DDH Lift coordination. |
...
| **Stale Slot Purge** (`pfStaleSlotDays`/`afStaleSlotDays`) | Slots with no new close in this many days are fully removed from the scorecard on the next trim. Keeps the scorecard clean when criteria combinations fall out of use. Plain number field, no upper bound. |
...
The **Both / Hist / Traded** source filter (`pfScorecardSource`/`afScorecardSource`) cycles through which score records are included in the displayed chips. **Hist** is retained for viewing old historical records, while new MIW/MIC historical scoring generation has been removed. This is a display-only filter — it does not affect which records count toward collapsed scoring. Selection is persisted.
...
- **MultiIndicator-Winter.html (v1.70.0) / MultiIndicator-Chaser.html (v1.69.0) / Permafrost-Winter.html (v1.59.0) / Ashfall-Chaser.html (v1.59.0)** — Added: `pecirr` (🔄) and `vsgirr` (🔀), a new "irregularity" criterion pair measuring price/volume "spikiness" — an efficiency-ratio read (net window high/low range vs. summed per-candle travel for price; net volume range vs. summed candle-to-candle volume swings for volume) computed from the same candle data PEC/VSG already fetch, no new fetch path. Gated by a new shared **Irregularity Threshold** config (default 60). Wired into both bots' full criteria surface (secondary criteria, blockable-by-default list, UI builder arrays, Force-Evaluate General Criteria topup, badge key/scorecard emoji, tag descriptions) and into Permafrost/Ashfall's independent Universe Weighing prescreen scoring.
  - Also adjusted `pecmomup`/`pecmomdn`/`vsgmomup`/`vsgmomdn`: once a window's average irregularity clears the threshold, a strict monotonic 6h/12h/24h ordering no longer auto-qualifies as momentum on its own — it falls into the existing nearest-equivalent tie-break scoring instead, with momentum's score linearly dampened toward zero as irregularity climbs from the threshold toward 100. Applied identically to MIW/MIC's live gate-check and to Permafrost/Ashfall's independent prescreen shape-match copies, so the criterion means the same thing in both places. `pecalpha`/`pecdelta`/`vsgalpha`/`vsgdelta` are unaffected.
- **Permafrost-Winter.html (v1.59.0) / Ashfall-Chaser.html (v1.59.0)** — `ocs` (Buy/Sell Skew) changed from reading only the most recent OC fetch batch to an accumulated buy/sell fill-count sum for that ticker across the entire retained OC cycle history (up to 25 cycles — the same window OCX's population average already uses), via new `_pfOcAccumSkew`/`_afOcAccumSkew` functions. A fresh live single-ticker sample (`_ocFetchSingle`, which deliberately never writes into cycle history) is tagged `live: true` and folded into the accumulated total exactly once, without double-counting once a later batch fetch pushes that ticker into history for real. `ocx` (Order Count Deviation) is unchanged — still reads only the latest batch's average interval against the population average.

- **MultiIndicator + Permafrost/Ashfall emoji cleanup** — Changed the mB-Liq badge from 🌨️ to 🧊 across MIW/MIC and Permafrost/Ashfall displays to avoid visual overlap with Regime cloud/tornado emojis.
- **MultiIndicator-Winter.html / MultiIndicator-Chaser.html** — Refreshed the slot-builder badge key to remove retired V/M, LSA/LBA, R-ASL/R-ABL, IO/T, and IO/M entries; added V/A, VSG, IO/A, and Regime badge documentation in both the plugin UI and README.
- **Permafrost-Winter.html / Ashfall-Chaser.html** — Extracted the analytics-side Gains Continuation reaction, partner live-liq/0-liq-win reaction toggles, Include Historical Scores toggle, and Presence Regime threshold/window/mixed controls into `junk/Permafrost-Ashfall-cross-score-regime-scrap.html`. Production Permafrost/Ashfall no longer act on partner gains-lock broadcasts, no longer expose cross-graylist reaction controls, always build scorecards from retained score records without the include-hist filter, and compute Regime skew from the retained liq history without configurable Presence Regime threshold/window/mixed handling.
- **MultiIndicator-Winter.html / MultiIndicator-Chaser.html** — Extracted the retired Multi-Indicator Fade config tab (Historical Scoring, Self-Liq Graylist, Direction Lock) and the exit-tab Protect Path-A substitution sub-toggle into `junk/MultiIndicator-fade-protectpathA-scrap.html`. Production MIW/MIC no longer render the Fade tab, no longer generate historical-score records, no longer run self-liq/direction-lock/partner-win graylist gates, and live substitution now uses score/age/margin/winner protection without a path-A shield.
- **MultiIndicator-Winter.html (v1.67.0) / MultiIndicator-Chaser.html (v1.66.0)** — Added: **Regime**, a new six-criterion group (`regimeslo`/`regimeblo`/`regimesmid`/`regimebmid`/`regimeshi`/`regimebhi`) reading Permafrost/Ashfall's population-wide liquidation skew directly — the first criteria group not derived from the ticker being evaluated. See **Regime** under the MultiIndicator Plugin section above for the full mechanics. Blockable by default (`va`/`ioa`-style reasoning — see **Blockable Criteria**), migration-seeded into existing saved `miwSlotBlockCriteria`/`micSlotBlockCriteria` the same way `va`/`ioa` were. Two new config sliders, **Regime Mid/Hi Threshold** (defaults 60/80). No new fetch path and no scan-cap scope — reads `_pfRegimeSkew`/`_afRegimeSkew` directly.
  - Caught during development, fixed before release: the shared annotate/tagging functions default any unmatched criterion to a bare tag with no `+`/`-` suffix, but the Slot Blocking check (`_miwIsEntryBlocked`/`_micIsEntryBlocked`) requires that suffix to match — the same class of bug that previously affected `fund`'s bare-tag PEC annotation. Regime's annotate branches were fixed to always emit a signed tag before this shipped, so Blockable Criteria works correctly for Regime from the first release rather than silently no-op'ing.
  - Depth recording was revised once after initial release: the numeric suffix is now the dominant share's percentage-point offset above its own band's floor, signed by direction (positive = sell-dominant, negative = buy-dominant) — e.g. `regimeslo+9` for 59% sliq, `regimeblo-1` for 51% bliq, `regimeshi+1` for 81% (measured from the Hi floor, not from 50). The first release recorded every match as a flat `+0`, which — since sign carried no directional meaning yet — made sell- and buy-dominant matches at the same emoji/band visually indistinguishable in scorecard logs.
- **Permafrost-Winter.html (v1.57.1) / Ashfall-Chaser.html (v1.57.1)** — Added Regime emoji recognition (🌤️/☁️/🌪️) to the tag-rendering dictionary used to display MIW/MIC scorecard tags in Permafrost/Ashfall's own UI. No functional changes on the Permafrost/Ashfall side — Regime's actual skew computation is available via `_pfRegimeSkew`/`_afRegimeSkew`.
- **Permafrost-Winter.html / Ashfall-Chaser.html (v1.60.0)** — Added **Partner Score Push** (`pfPartnerScoreMode`/`afPartnerScoreMode`): a global three-way toggle (Only Wins default / Only Losses / Wins-Losses) gating which partner-bot outcomes are allowed to move a criterion's collapsed score. Addresses an unbounded see-saw where a criterion with no stable edge would oscillate between blocked and unblocked purely off alternating partner wins and losses. Applies uniformly across all three scoring passes — the scoreboard map, the collapsed rank/critStats that Slot Blocking and Prospect Pre-Screening read from, and the Blind-Entry/Liquid-Diver bucket. Own-bot trades are never filtered. A partner close excluded from scoring still counts toward Stale Slot Purge freshness, so a criterion the partner keeps trading isn't purged just because its outcomes are currently filtered out of scoring. **Stale Slot Purge** (`pfStaleSlotDays`/`afStaleSlotDays`) changed from a 1–90 day slider to a plain unbounded number field.
- **Permafrost-Winter.html / Ashfall-Chaser.html (v1.60.1)** — Fixed: the new Partner Score Push button used `justify-content:space-between`, pushing it to the far edge of the settings panel where it collided with the scrollbar gutter. Moved to a left-aligned block below its label and enlarged, matching the panel's other controls.

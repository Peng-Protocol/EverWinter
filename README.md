...
**80% margin floor on absorption cuts:** Each absorption cut is limited to at most 20% of the position's current margin. If the base cut size would leave less than 80% of current margin remaining, the cut is skipped for that cycle. This prevents repeated absorption from reducing margin to a level where the EDa TP recovery target exceeds the entry price — a condition that would silently zero out `_edaTpPrice` and allow the normal 3% reduce TP to fire before accumulated losses are recovered.

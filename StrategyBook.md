...
Edge lives at the ends of a reading's own range, not in the middle of it. Every criterion the system watches is sorted continuously into three bands against its own recent history — an extreme high end, an extreme low end, and the lukewarm middle where a nominally favorable reading means very little. Both ends are worth trading; which one is worth trading right now is the part nobody can know in advance.

Whether extreme readings or lukewarm readings are the bullish case, and which are the bearish one, depends entirely on the prevailing market regime — and the regime is unknowable. It cannot be read off in advance and it cannot be predicted. What is observable is that once a regime is actually in effect, only one end of the spectrum pays: extremes and lukewarms do not both produce profit in the same conditions, and the losing end keeps losing for as long as that regime holds. So the system does not guess. It keeps a running tally of both ends, targets whichever has actually been in profit, and passes over any candidate leaning too heavily on the other one. When the tally flips, the target flips with it — and open positions are held to the same standard for their whole life, closed once they drift onto the side that is currently losing.

The same logic runs one level down, per criterion. A reading that only pays at its high end should not be credited for a win it took at its low end, and pooling both into one average hides the difference until the criterion just looks mediocre everywhere. Each end is scored separately, so a criterion earns trust at the end where it is genuinely working and loses it at the end where it is not.

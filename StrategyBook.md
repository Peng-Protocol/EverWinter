...
- **Price shape** — Where has this coin's price actually been today, not just up or down but in what shape? Each reading below looks at three different windows at once — the last six hours, the last twelve, and the full day — and compares them against each other, giving a rough shape of the day rather than a single up/down number. Nothing here needs waiting for a candle to close; it reads directly off however much of the day has already happened.
  ...
  - **PEC Irregular** — This isn't a shape comparison like the five above; it asks a different question entirely: is the price actually going somewhere, or just churning in place? A window where price wanders back and forth over the same band reads irregular even if it never touches an extreme; a window that moves steadily in one direction, even a small one, reads the opposite. Once a ticker's day is irregular enough, Bullish/Bearish Momentum stop taking a monotonic-looking sequence at face value — a shape that happens to line up 6h > 12h > 24h in a genuinely choppy market is closer to coincidence than trend, so momentum's confidence fades the choppier the day gets, gradually rather than all at once. Red Mountain/Green Delta aren't touched by this — a hump or dip shape is judged on its own regardless of how spikey the rest of the day was.
...
- **Volume shape** — Where has this coin's trade volume actually been today, not just one hour's spike but in what shape? Same three windows as price shape above — the last six hours, the last twelve, and the full day — each measured against its own recent range and compared to the others, giving a shape reading of trade activity instead of a single hour's multiplier against a flat daily average.
  - **VSG Over-Traded** — Every window, short and long, shows unusually heavy activity at once. Sustained heavy participation across the whole day, not one busy hour.
  - **VSG Under-Traded** — The mirror: every window has stayed quiet relative to its own recent range. The market has gone quiet across the board, not just briefly.
  - **VSG Mountain** — The middle stretch of the day traded busier than both the start of the day and right now — a burst of activity that's already crested.
  - **VSG Trough** — The mirror: the middle of the day went quieter than both the start and now — a lull, picking back up on both sides.
  - **VSG Building Momentum** — The most recent hours are busier than the half-day, which is busier than the full day — activity building all day, loudest right now.
  - **VSG Fading Momentum** — The mirror: the most recent hours are quieter than the half-day, which is quieter than the full day — activity tapering off, quietest right now.
  - **VSG Irregular** — Same idea as PEC Irregular above, applied to trade activity instead of price: is participation actually trending toward one extreme, or just spiking up and down erratically without settling anywhere? A day of activity that lurches between very busy and very quiet reads irregular even if it never sits at either extreme for long. Same effect on the momentum pair above — Building/Fading Momentum's confidence fades the more erratic the day's activity has been, while VSG Mountain/Trough are unaffected.

  Unlike the old spike readings, volume shape doesn't tell you which way the candle closed — it only tells you that trade activity itself is doing something unusual. Pair it with a liquidation reading, funding, or Buy/Sell Skew for direction; use volume shape purely as confirmation that real participation backs the move, not as the directional signal itself.
...
- **Regime** — Every reading above asks something about the specific ticker under consideration. Regime asks something about the market as a whole instead: what share of *all* recent liquidation turnover, across every ticker being watched, has landed on one side versus the other, banded into low/mid/high conviction and signed by which side is dominant. This is the same liquidation-skew read described in Market Intelligence below as the system's most potent regime gauge — Regime is what turns that qualitative read into something the scorecard can score and trade on directly, rather than something only consulted by feel. Because it's read once per cycle rather than once per ticker, every candidate in the same scan sees the same Regime reading — it won't tell you which specific ticker to prefer, but it tells you whether the broader liquidation backdrop right now favors this side's thesis at all, and lets losing bands or directions retire themselves the same way any other criterion does.

**Building slots**: A slot containing only "S-Liq" behaves like a bare liquidation filter — no confirmation, just the base signal. A slot containing "S-Liq" and "V/A>10" requires the liquidation to also come with unusually high participation relative to the market. A slot with "S-Liq", "V/A>10", and "fund<-1" requires liquidation, participation, and a funding premium to all align before opening. Adding "VSG Under-Traded" to that same slot demands the broader volume backdrop confirm the move too. Each addition narrows the filter from permissive to strict without changing the underlying logic — but the liquidation reading itself is never optional, it's the base every slot is built on. The one exception is Past Liquidation: since that reading is really about *which tickers the slot gets tested against* rather than a condition written into the slot, a Past Liquidation slot is built entirely from secondary criteria with no liquidation reading named in it at all — the pool restriction does that job instead.

**Auto-slot builder**: Instead of building slots by hand, the system can generate the entry gate automatically from a chosen minimum: a ticker qualifies once it satisfies a liquidation reading (or the Past Liquidation pool restriction) plus at least that many secondary criteria — a floor, not a fixed count. A ticker satisfying more conditions than the minimum is credited for everything it actually exhibits, not sliced down to only the first few that happen to clear the floor — a ticker showing five genuine confirming conditions is scored on all five, not an arbitrary subset. When Past Liquidation is one of the active readings, the same floor applies to secondary-only qualification, since there's no liquidation reading to anchor on there — see **Building slots** above. Tiered criteria (Fund, V/A, IO/A, Order Count Deviation, Regime) record the specific tier value at entry, so even a single qualifying condition can produce many distinct scorecard entries depending on how strong the reading was at each trade — and because a ticker's full exhibited set is scored rather than a fixed slice of it, the tracked variety reflects everything that was actually true at entry. Some secondary criteria are inherently opposed and cannot coexist: the three price-shape opposite pairs (PEC Overbought/Oversold, Red Mountain/Green Delta, Bullish/Bearish Momentum), the equivalent volume-shape pairs (VSG Mountain/Trough, VSG Building/Fading Momentum) — a window's volume shape only crests, troughs, builds, or fades in one direction at once — and Regime, which is exclusive across all six of its own readings at once rather than just within an opposite pair, since only one band/side combination can be true off a single population-wide reading per cycle. Combined with auto-correction, this creates a self-pruning strategy: every genuinely satisfied condition counts toward the scorecard, and the ones that consistently lose are disabled without manual intervention. Because Buy/Sell Skew and Order Count Deviation are backed by data that is essentially always present, expect them to appear in a large share of matched positions once order flow tracking is turned on — that is expected behavior, not over-triggering.
...
The system learns from its own closed positions — and from simulated positions that were never actually opened — which entry conditions have been profitable and which have not. Combinations that win rise to the top of the entry queue; consistently losing combinations are deprioritized. A further layer sits underneath this: a signal is only trusted once its own readings run genuinely hot relative to its recent history, not just nominally favorable — a candidate leaning mostly on lukewarm, middling readings is passed over outright, regardless of what the raw win/loss record on those readings looks like.
...
The **Regime** reading (see Market Reading above) is this same gauge applied one level up — not whether *this* ticker liquidated, but which side is dominating liquidation turnover across the whole watched market right now, and how strongly. Where every other criterion in this system waits for the scorecard to tell it whether a per-ticker read is working, Regime lets the scorecard evaluate the market-wide read directly, band by band and side by side, rather than trusting it by reputation.
...
**Regime Based Scoring** takes this a step further: a criterion is rarely equally reliable across every market structure. A momentum read that wins convincingly while one side is decisively dominating liquidation turnover can just as easily be a coin-flip, or a quiet loser, while turnover is closer to even — and a single pooled score across all history hides that difference entirely, averaging a criterion's best conditions together with its worst until it just looks mediocre everywhere. Certain criteria are only genuinely profitable under certain market structures; the edge was always there, just diluted by conditions where it never applied. Rather than learning one score per criterion for all time, Regime Based Scoring learns six — one for each Low/Mid/High, Bull/Bear band Regime itself reads — so a combination only gets credit or blame for how it performed under the structure it actually traded in. Substitution and entry ordering follow whichever regime-specific record matches the market right now rather than the flattened long-run average, so the queue reflects what's actually working under today's conditions, not a number smoothed out by every other condition the market has been in. This only engages once Regime criteria are active in the entry gate in the first place — without a regime tag on the record, there's nothing to sort the learning by.

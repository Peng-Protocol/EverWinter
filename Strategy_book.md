...
The newer regime read recontextualizes the whole intelligence stack around liquidation. Treat the rolling liquidation sample as the market's governing state and every other indicator as secondary unless the scorecard proves otherwise. In the observed data, the market behaves as if there are two liquidation-defined regimes:

- **Broadly bullish liquidation regime** — the retained sample window is dominated by bullish liquidation flow. In this state, a live liquidation event is best treated as a long trigger once its turnover is significant relative to the average liquidation turnover in the retained samples. The mirror path is absence: a ticker with no liquidation event across the sample window becomes a short candidate, because it failed to participate in the regime's forced-buying pressure.
- **Broadly bearish liquidation regime** — the retained sample window is dominated by bearish liquidation flow. In this state, a live liquidation event is best treated as a short trigger once its turnover is significant relative to the average liquidation turnover in the retained samples. The absence path inverts: a ticker with no liquidation event across the sample window becomes a long candidate, because it failed to participate in the regime's forced-selling pressure.

This does not mean every liquidation print is automatically tradeable. "Significant" matters: the event should clear a depth/turnover threshold against the bot's own retained liquidation history, not just register a tiny forced trade. It also means the old mental model — S-Liq is always bullish, B-Liq is always bearish — is incomplete. The ticker-level liquidation type still describes what happened locally, but the regime decides whether liquidation presence itself is the side to follow or the side to fade. In practice, Auto Liquidation Presence is the regime switch: in a bullish liquidation regime, run the live-liq path for the long side and the 0-Liq path for the short side; in a bearish liquidation regime, run the live-liq path for the short side and the 0-Liq path for the long side.
...
- **S-Liq (Sell-Liquidation)** — Short positions on this ticker account for at least 70% of its liquidation turnover so far — a decisively one-sided reading, acted on the instant it crosses that line rather than waiting for anything to close. In the old local-only interpretation, short liquidations are bullish because the market forced out the short side with conviction. In the regime interpretation, S-Liq is part of the broader live-liq path: follow it when the retained liquidation sample says live liquidations are the profitable side, and fade/avoid it when the sample says live liquidations are hostile to this bot's direction. An optional depth gate can additionally require the liquidated value to sit a set percentage above or below the average liquidation size seen across retained history — a read on whether this event is actually large enough to matter.
- **B-Liq (Buy-Liquidation)** — Long positions on this ticker account for at least 70% of its liquidation turnover, same immediate reaction as S-Liq. In the old local-only interpretation, long liquidations are bearish because the market forced out the long side with conviction. In the regime interpretation, B-Liq is also part of the broader live-liq path: its usefulness depends less on the label alone and more on whether the current retained sample says liquidation presence should be followed or faded. The same optional depth gate applies.
- **mS-Liq (Mixed Sell-Liquidation)** — On this ticker, sell liquidations lead, but buy liquidations still hold at least 30% of its turnover — a contested reading with a sell lean, same immediate reaction. Use this as a weaker live-liq event when a clean one-sided signal is absent, and let the regime decide whether live-liq presence belongs to this bot or its partner. The same depth gate applies.
- **mB-Liq (Mixed Buy-Liquidation)** — On this ticker, buy liquidations lead, but sell liquidations still hold at least 30% — a contested reading with a buy lean, same immediate reaction. Use this as the mixed buy-lean counterpart to mS-Liq: still a live-liq event, still subordinate to the retained sample's bullish/bearish regime read.
- **0-Liq (No Liquidation)** — Nothing was liquidated on this ticker for the entire cycle — the mirror case to every live liquidation reading above. Unlike the four above, there's no way to observe an absence in real time, so this one only resolves once the cycle finishes. Under the regime interpretation, 0-Liq is not "no signal"; it is the absence side of the liquidation regime. If live liquidation presence is broadly bullish, 0-Liq becomes the short-side path. If live liquidation presence is broadly bearish, 0-Liq becomes the long-side path.
...
**Auto-slot builder**: Instead of building slots by hand, the system can generate every combination of a liquidation reading plus secondary criteria at a chosen size automatically — one liquidation anchor times however many secondary combinations fit the remaining size. Tiered criteria (Fund, V/M, spike, IO/T, IO/M) record the specific tier value at entry, so a single base combination like "S-Liq + fund>1" can produce dozens of distinct scorecard entries depending on how far the funding rate was at each trade. The effective tracked variety grows with data and can far exceed the base combination count. Some secondary criteria are inherently opposed and cannot coexist: +24h and -24h, and any two from the candle group (lsa, lba, rasl, rabl) — a candle can only close in one direction with one volume character simultaneously. Auto-generated slots containing any such pair will never see a position. Combined with auto-correction, this creates a self-pruning strategy: all valid combinations run, and the ones that consistently lose are disabled without manual intervention. Because Buy/Sell Skew and Order Count Deviation are backed by data that is essentially always present, expect them to appear in a large share of matched positions once order flow tracking is turned on — that is expected behavior, not over-triggering.

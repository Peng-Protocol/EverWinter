...
**Fade Away** — Three signals feed a combined adverse score each scan: candle structure, liquidation flow, and funding rate direction. When the combined reading crosses the threshold, the oldest open position closes; with entry blocking on, new positions also pause until conditions clear. Candle structure contributes unconditionally — when the tape is broadly moving against the open position direction, that counts against it. Liquidation and funding signals are gated by scorecard history: they only contribute when past trades opened under the same entry conditions confirm the signal has historically led to losses. If the history is absent or ambiguous those signals stay silent. A loss trigger can also fire the same evaluation between scans when any position is already in loss past a set amount, so a deteriorating position does not have to wait for the next scan cycle.

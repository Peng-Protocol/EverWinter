...
When a single position has grown disproportionate relative to the rest of the book — whether through multiple DCA stages, an unusually deep loss, or both — standard absorption at its normal halving interval may leave it festering too long. The outstanding system identifies positions whose margin or absolute loss exceeds a configured multiple of the average across all other open positions (default 2.5×) and immediately locks their absorption to the 30-second minimum cooldown. The halving schedule is bypassed entirely for the duration of the outstanding condition; as soon as the position falls back within normal range, the standard cooldown resumes.

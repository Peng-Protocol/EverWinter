...
- **Order Count Deviation** — How a ticker's total order flow compares to the average across the surveilled sample, above or below. A ticker trading well above the pack is drawing outsized attention right now; one trading well below it is quiet relative to peers. Same always-on caveat as Buy/Sell Skew: this is one of the few signals with no downtime, so it shows up often and should be weighted as a routine confirming factor, not a special case.
